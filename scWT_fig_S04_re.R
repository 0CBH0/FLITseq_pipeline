...
data_raw <- H5File$new("./3T3/scWT_02_juncs_info.h5", mode="r")
sct <- list()
...
data_raw$close_all()
scta <- sct[["features"]]
data_raw <- H5File$new("./3T3/scWT_03_juncs_info.h5", mode="r")
sct <- list()
sparse.mat <- sparseMatrix(i=data_raw[["matrix/indices"]][]+1, p=data_raw[["matrix/indptr"]][], 
	x=as.numeric(data_raw[["matrix/data"]][]), dims=data_raw[["matrix/shape"]][], repr = "T")
rownames(sparse.mat) <- data_raw[["matrix/genes"]][]
...
junc_bulk <- read.delim("./3T3/3T3_juncs_info.tsv")[, -2]
...
write.table(junc_info, "3T3/juncs_info_count.txt", quote=F, sep="\t")
...
write.table(gene_info, "3T3/juncs_info_all.txt", quote=F, sep="\t")


gene_list <- c("Prl2c2", "Myl6")
junc_list <- c("ENSMUSG00000079092.5", "ENSMUSG00000090841.3")

rec <- read.delim("./3T3/juncs_info_count.txt")
rec$SC <- log2(rec$SC)
rec$BK <- log2(rec$BK)
...
ggsave(plot=pa, width=3.5, height=3, dpi=300, "scFLIT_FigS04_A.png", limitsize=F)
ggsave(plot=pa, width=3.5, height=3, dpi=300, "scFLIT_FigS04_A.pdf", limitsize=F)

trans_info <- read.delim("./3T3/juncs_info_all.txt")
...
pb <- wrap_elements(pb)
ggsave(plot=pb, width=3.5, height=3, dpi=300, "scFLIT_FigS04_B.png", limitsize=F)
ggsave(plot=pb, width=3.5, height=3, dpi=300, "scFLIT_FigS04_B.pdf", limitsize=F)


density_info <- read.csv("./3T3/res_info_density.csv", h=T)
coord_info <- read.csv("./3T3/res_info_coord.csv", h=T)
junction_info <- read.csv("./3T3/res_info_junction.csv", h=T)
...
ggsave(plot=pc, width=8.5, height=5, dpi=300, "scFLIT_FigS04_C.png", limitsize=F)
ggsave(plot=pc, width=8.5, height=5, dpi=300, "scFLIT_FigS04_C.pdf", limitsize=F)
