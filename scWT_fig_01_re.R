...
term <- read.csv("bulk_met/878_80%_tso.csv", r=1, h=T)
...
ggsave(plot=pb, width=3.5, height=2, dpi=300, "scFLIT_Fig01_B.png", limitsize=F)
ggsave(plot=pb, width=3.5, height=2, dpi=300, "scFLIT_Fig01_B.pdf", limitsize=F)
...
term <- read.csv("bulk_met/12.25_40%.csv", r=1, h=T)
term$Signal <- filter(term$Signal)
tb <- median(term$Signal)
tn <- abs(mean(term$Signal[which(term$Signal < tb)]) - tb)
noise_total <- (noise + tn)/2
...
pb <- wrap_elements(ggplot(term, aes(x=Scale, y=Signal))+geom_line(linewidth=0.7, colour=col_list[4])+
	...
	legend.key.size=unit(8, "pt"), legend.box.spacing = unit(0, "pt"), 
	legend.key=element_blank(), legend.background=element_blank()))
ggsave(plot=pb, width=3.5, height=2, dpi=300, "scFLIT_Fig01_C.png", limitsize=F)
ggsave(plot=pb, width=3.5, height=2, dpi=300, "scFLIT_Fig01_C.pdf", limitsize=F)
